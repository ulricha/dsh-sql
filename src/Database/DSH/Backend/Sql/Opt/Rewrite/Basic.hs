...
constFilteringJoinPred q =
  $(dagPatMatch 'q "(q1) [AntiJoin | SemiJoin]@joinOp p (q2)"
    [| do
        constColsLeft  <- pConst . bu <$> properties $(v "q1")
        constColsRight <- pConst . bu <$> properties $(v "q2")
        let p' = deleteWith (constTrueJoinConjunct constColsLeft constColsRight) $(v "p")
        predicate $ length p' < length $(v "p")
        predicate $ not $ null p'

        return $ do
            logRewrite "Basic.Const.AntiJoin/SemiJoin.Predicate" q
...
pullProjectRownum q =
    $(dagPatMatch 'q "RowNum s (Project p (q1))"
      [| do
          ...
          let requiredCols = S.unions $ map (exprCols . snd) p'

          -- Make sure that the rownum result column does not appear
          -- in one of the projection expressions to avoid shadowing.
          predicate $ not $ S.member resCol requiredCols

          return $ do
              logRewrite "Basic.PullProject.Rownum" q
              let p'' = p' ++ [(resCol, ColE resCol)]
                  s' = mergeProjIntoSortSpec $(v "s") $(v "p")
                  inProj = map (\c -> (c, ColE c)) $ S.toList requiredCols
              -- Keep only those columns of the input that are actually used by
              -- the original projection. This avoids conflicts between columns
              -- removed by the original projection and the rownum output
              -- column.
              projNode   <- insert $ UnOp (Project inProj) $(v "q1")
              numNode <- insert $ UnOp (RowNum s') projNode
              topNode <- replaceWithNew q $ UnOp (Project p'') numNode
              traceM $ printf "q1: %d" $(v "q1")
              traceM $ printf "projNode: %d" projNode
              traceM $ printf "numNode: %d" numNode
              traceM $ printf "topProj: %d" topNode
              d <- exposeDag
              traceM $ show $ D.nodeMap d
       |])
...
pullProjectThetaJoinRight q =
    $(dagPatMatch 'q "(q1) ThetaJoin jp (Project p (q2))"
      [| do
          colsLeft  <- fmap fst . pCols . bu <$> properties $(v "q1")
          colsRight <- fmap fst . pCols . bu <$> properties $(v "q2")
          predicate $ S.null $ S.intersection colsLeft colsRight
          ...
          return $ do
              logRewrite "Basic.PullProject.ThetaJoin.Right" q
              let jp' = inlineJoinPredRight $(v "p") $(v "jp")
                  p'  = S.toList (fmap (\c -> (c, ColE c)) colsLeft)
                        ++
                        $(v "p")
              joinNode <- insert $ BinOp (ThetaJoin jp') $(v "q1") $(v "q2")
              void $ replaceWithNew q $ UnOp (Project p') joinNode |])
...
inlineProjectAggr q =
    $(dagPatMatch 'q "Aggr args (Project p (q1))"
      [| do
          ...
          return $ do
              logRewrite "Basic.PullProject.Aggr.Inline" q
